...
Также есть методы [`vector.insert(element)`](https://en.cppreference.com/w/cpp/container/vector/insert) и [`vector.erase(pointer)`](https://en.cppreference.com/w/cpp/container/vector/erase), с ними вы можете ознакомиться по ссылкам. Эти методы работают слишком долго и поэтому используются редко.
...
Чтобы отсортировать вектор **по убыванию**, достаточно поменять begin -> rbegin(reversed begin), end -> rend(reversed end), см.[картинку](#Set):  
`sort(v.begin(), v.end())` - в качестве параметров мы передали начало и конец вектора в памяти(указатели на начало и конец)  
